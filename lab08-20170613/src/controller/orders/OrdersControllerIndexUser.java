...
public class OrdersControllerIndexUser extends HttpServlet {	
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		...
		if(Usuario == null){
			...
			String query3 = "select from " + Order.class.getName()+"where=="+Usuario.getEmail(); 
  		   List<Order> order = (List<Order>) pm.newQuery(query3).execute();
  		   req.setAttribute("orders",order);   
			RequestDispatcher rd = req.getRequestDispatcher("/WEB-INF/Views/Order/orderIndexUser.jsp");  
    		rd.forward(req, resp);		
		}
		else {

			try {
				req.getRequestDispatcher("/WEB-INF/Views/Error/Error1.jsp").forward(req, resp);

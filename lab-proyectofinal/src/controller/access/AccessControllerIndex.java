...
public class AccessControllerIndex extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if(Usuario == null){
			...
			List<Access> listAccess = (List<Access>)query0.execute("select from Acceso");

			List<AccessAuxiliar> array = new ArrayList<AccessAuxiliar>();
			AccessAuxiliar nuevo;

			for(Access acceso:listAccess){
				...
				nuevo = new AccessAuxiliar(acceso.getId(),rol.getNombre(),recurso.getUrl(),acceso.isEstado());
				array.add(nuevo);
			}
